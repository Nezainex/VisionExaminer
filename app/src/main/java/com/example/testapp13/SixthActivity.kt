...
class SixthActivity : AppCompatActivity() {
    ...
    private lateinit var answerButtonsrabkin: List<Button>
    ...
    private val rabkinImages = listOf(
        ...
        RabkinImage(
            R.drawable.rabkin18,
            "Одноцветными являются горизонтальные ряды: 9-й, 10-й, 11-й, 12-й, 13-й, 14-й, 15-й и 16-й",
            "Одноцветными являются вертикальные цветовые ряды: 3-й, 5-й и 7-й",
            "Одноцветными являются вертикальные цветовые ряды: 1-й, 2-й, 4-й, 6-й и 8-й",
            "Одноцветными являются горизонтальные и вертикальные ряды",
        ),
        ...
        RabkinImage(
            R.drawable.rabkin22,
            "Одноцветными являются вертикальные ряды, горизонтальные - цветные",
            "Одноцветными являются горизонтальные ряды, вертикальные - разноцветные",
    ...
    private fun showQuestion() {
        ...
        for (i in 0..3) {
            answerButtonsrabkin[i].text = shuffledAnswers[i]
            answerButtonsrabkin[i].setOnClickListener {
    ...
    private fun onAnswerClick(selectedAnswer: String, correctAnswer: String) {
        ...
        if (selectedAnswer == correctAnswer) {
            ...
            // "Не могу сказать" и остальные неправильные ответы засчитываются как ошибка
            when (selectedAnswer) {
    ...
    private fun showResult() {
        val resultText = when (totalScore) {
            in 0..10 -> "Нормохромазия"
            in 11..16 -> "Аномальная трихромазия (слабая степень)"
            in 17..21 -> "Аномальная трихромазия (средняя степень)"
            in 22..26 -> "Аномальная трихромазия (сильная степень)"
            else -> "Монохромазия или симуляция" // Убрано условие для 27 баллов, так как оно избыточно
        }

        // Логика для определения и отображения конкретного типа аномалии
        var deficiencyType = ""
    ...
    private fun enableFinishButton() {
        val finishTestButton = findViewById<Button>(R.id.finishTestButtonrabkin)
    ...
    private fun resetQuiz() {
        ...
        resultTextViewrabkin.text = ""  // Очищаем resultTextView
    }
    ...
    private fun saverabkinResultAndFinish() {
        ...
        val rabkinResult = RabkinResult(
            patientProfileId = -1, // Временное значение
            score = score,
            resultText = resultText
        )

        // Возврат во SecondActivity
        val intent = Intent()
        intent.putExtra("rabkinResult", rabkinResult) // Передача объекта RabkinResult
